...
public class MessagesController extends BaseController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public boolean isPeerBlocked(TLRPC.Peer peer) {
        if(peer.user_id != 0) {
            return isUserBlocked(peer.user_id);
    ...
    public boolean isUserBlocked(int user_id) {
        if(!(blockedEndReached || loadingBlockedPeers)) {
            getBlockedPeers(false);
        }
        if (blockePeers == null || totalBlockedCount < 0) {
            return false;
        }
        return blockePeers.indexOfKey(user_id) >= 0;
    }
